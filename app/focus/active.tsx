...
export default function ActiveFocusScreen() {
  ...
  const handleEndEarly = useCallback(() => {
    if (activeSession?.phase === 'work') {
      ...
      if (elapsedMins > 0) {
        console.log('[FocusActive] End early — recording partial progress:', elapsedMins, 'min');
  ...
  const handleStopAndExit = useCallback(() => {
    if (activeSession?.phase === 'work') {
      const elapsedMins = getElapsedMinutes();
      if (elapsedMins > 0) {
        console.log('[FocusActive] Stop & exit — recording partial progress:', elapsedMins, 'min');
        recordSession({ goalId: activeSession.goalId, minutes: elapsedMins });
      }
    }
    endSession();
  ...
  const getBackgroundColor = () => {
    ...
    switch (activeSession.phase) {
      case 'work': return theme.background;
      case 'break': return colorScheme === 'dark' ? '#064E3B' : '#F0FDF4';
      case 'long-break': return colorScheme === 'dark' ? '#134E4A' : '#F0FDFA';
    ...
    return (
      <SafeAreaView style={[styles.container, { backgroundColor: theme.background }]}>
        <View style={styles.summaryOverlay}>
          ...
          <Text style={[styles.summaryTitle, { color: theme.text }]}>Session Complete!</Text>
          <Text style={[styles.summarySubtitle, { color: theme.icon }]}>Great job staying focused.</Text>
          
          <View style={styles.summaryStats}>
            <View style={[styles.summaryStatCard, { backgroundColor: theme.card }]}>
              <Text style={[styles.statValue, { color: theme.text }]}>{activeSession.pomodorosCompleted}</Text>
              <Text style={[styles.statLabel, { color: theme.icon }]}>Pomodoros</Text>
            </View>
            <View style={[styles.summaryStatCard, { backgroundColor: theme.card }]}>
              <Text style={[styles.statValue, { color: theme.text }]}>{activeSession.pomodorosCompleted * activeSession.workDuration}m</Text>
              <Text style={[styles.statLabel, { color: theme.icon }]}>Focused</Text>
            </View>
          </View>
    ...
    <SafeAreaView style={[styles.container, { backgroundColor: getBackgroundColor() }]}>
      <View style={styles.header}>
        ...
        <View style={styles.headerInfo}>
          <Text style={[styles.goalName, { color: theme.text }]}>{activeSession.goalName}</Text>
          <Text style={[styles.sessionStatus, { color: theme.icon }]}>
            Pomodoro {activeSession.pomodorosCompleted + 1} of {activeSession.totalPomodoros}
          </Text>
        </View>
        ...
        <TouchableOpacity 
          onPress={() => {
            ...
            Alert.alert("End Session", "End this session or minimize?", [
              ...
              { text: "End Early", style: 'destructive', onPress: handleEndEarly },
      ...
      <View style={styles.controlsSection}>
        <View style={styles.mainControls}>
          ...
          >
            ...
            <Text style={[styles.controlLabel, { color: theme.text }]}>+5m</Text>
          </TouchableOpacity>

          <TouchableOpacity 
            style={[styles.playPauseButton, { backgroundColor: theme.primary }]}
            onPress={() => {
                console.log('[FocusActive] Play/Pause pressed, isPaused:', activeSession.isPaused);
          ...
          >
            ...
            <Text style={[styles.controlLabel, { color: theme.text }]}>Skip</Text>
          </TouchableOpacity>
        </View>
        ...
        <TouchableOpacity 
          ...
          onPress={() => {
            ...
            Alert.alert("Stop Session?", "You'll lose progress on this pomodoro.", [
              { text: "Cancel", style: 'cancel' },
              { text: "Stop & Exit", style: 'destructive', onPress: handleStopAndExit }
            ]);
          }}
        >
      ...
      <View style={[styles.footer, { borderTopColor: theme.card }]}>
        ...
        <View style={styles.stat}>
            <Text style={[styles.statValue, { color: theme.text }]}>
                {Math.round((activeSession.pomodorosCompleted * activeSession.workDuration) / 6) / 10}h
            </Text>
